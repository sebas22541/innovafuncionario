...
export async function generateAndStoreUserCredential(
  ...
  const pdfPublicId = `credencial-${token}`;
  ...
  const [, pdfUrl] = await Promise.all([
    ...
    uploadBufferToCloudinary({
      buffer: Buffer.from(pdfBytes),
      folder: "documentos/credenciales",
      publicId: `${pdfPublicId}.pdf`,
      resourceType: "raw",
      filename: `${pdfPublicId}.pdf`,
      mimetype: "application/pdf",
    }),
  ]);
...
function drawBackPageQr(page: any, input: {
  ...
  const { width, height } = page.getSize();
  const qrSize = Math.min(width * 0.30, height * 0.20);
